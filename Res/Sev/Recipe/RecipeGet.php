...
if(count($urlSegments)==2){
    ...
    $collection = ($db)->ATWD2->Recipe;
    $data = $collection->find([], ["skip" => $skip, "limit" => $limit, "sort" => ["title" => 1]]);
    $result = array();
    ...
    $db = $dbM->openConnection();
    $collection = ($db)->ATWD2->Recipe;
    $data = $collection->find(["title"=>$k], ["skip" => $skip, "limit" => $limit, "sort" => ["title" => 1]]);
    $result = array();
    array_push($result, $data->ToArray());
...
// $result = $song->find([], ["skip" => $skip, "limit" => $limit, "sort" => ["title" => 1]]);
// $data = array();
